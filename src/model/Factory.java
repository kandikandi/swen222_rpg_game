...
/**
 * This factory class is used to create most of the required state elements for
 * a game. Two map files are read. One is the Tile map, which has an array of
 * Tiles, which are "dumb" elements. The other map contains the game's Actors,
 * which include the Players, Enemies, Walls, Collectables, etc. The other
 * methods construct standard Actor objects with convenient syntax that only
 * requires a position for initial placement.
 */
...
public class Factory {
	 ...
	 * the required Actors instantiations, using the helper methods further down.
	 */
		...
		for (int row = 0; row < tileCode.length; row++) {
			for (int col = 0; col < tileCode[0].length; col++) {
				...
				case COIN:
					...
					break;
				case TREASURE:
					actor = createSpecialCoin(position.getxPos(),
							position.getyPos());
					break;
				case COINBAG:
				...
				case DOOR:
					...
					break;
				case SPECIAL_DOOR:
					actor = createSpecialDoor(position.getxPos(),
							position.getyPos(), true);
					break;
				case ENEMY:
	...
	/**
	 * This method constructs a standard Candy object.
	 */
